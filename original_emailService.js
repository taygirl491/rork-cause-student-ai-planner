	...
	try {
		...
		const emailPromises = existingMembers.map((member) => {
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0; font-size: 24px;">New Member Alert! ≡ƒÄë</h1>
            </div>
            
            <div style="background-color: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              <p style="font-size: 16px; color: #374151; line-height: 1.6;">
                Great news! Someone new has joined your study group <strong>${
									groupData.name
								}</strong>.
              </p>
              
              <div style="background-color: #f3f4f6; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #667eea;">
                <p style="margin: 0 0 10px 0; color: #1f2937;"><strong>≡ƒæñ New member(s):</strong> ${newMemberEmails}</p>
                <p style="margin: 0 0 10px 0; color: #1f2937;"><strong>≡ƒôÜ Group:</strong> ${
									groupData.name
								}</p>
                <p style="margin: 0 0 10px 0; color: #1f2937;"><strong>≡ƒôû Class:</strong> ${
									groupData.className || "N/A"
								}</p>
                <p style="margin: 0; color: #1f2937;"><strong>≡ƒÅ½ School:</strong> ${
									groupData.school || "N/A"
								}</p>
              </div>
        ...
        `,
				text: `New member joined ${
					groupData.name
				}: ${newMemberEmails}. Group: ${groupData.name}, Class: ${
					groupData.className || "N/A"
				}, School: ${
					groupData.school || "N/A"
		...
		console.log(
			`Γ£ô Sent ${emailPromises.length} join notifications for group ${groupData.name}`
		);
...
async function sendMessageNotification(groupData, message, recipients) {
	try {
		const emailPromises = recipients.map((member) => {
          ...
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0;">
              <h1 style="color: white; margin: 0; font-size: 24px;">≡ƒÆ¼ New Message in ${
								groupData.name
							}</h1>
            </div>
            ...
            <div style="background-color: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
              ...
              <div style="background-color: #f9fafb; padding: 20px; border-radius: 8px; border-left: 4px solid #667eea;">
                <p style="margin: 0; color: #374151; white-space: pre-wrap; line-height: 1.6; font-size: 15px;">${
									message.message
								}</p>
              </div>
              ...
              ${
                ...
                <div style="margin-top: 20px; background-color: #f0f9ff; border: 2px dashed #0ea5e9; border-radius: 8px; padding: 20px;">
                  <div style="display: flex; align-items: center; margin-bottom: 10px;">
                    <span style="font-size: 24px; margin-right: 10px;">≡ƒôÄ</span>
                  ...
                  ${message.attachments
                    ...
                    <div style="background-color: #ffffff; padding: 12px; margin: 8px 0; border-radius: 6px; border-left: 3px solid #0ea5e9;">
                      <p style="margin: 0; color: #1e293b; font-weight: 600; font-size: 14px;">≡ƒôä ${
												att.name
											}</p>
                  ...
                  <p style="margin: 15px 0 0 0; color: #0369a1; font-size: 13px; font-style: italic;">
                    ΓÜá∩╕Å Open the CauseAI app to view these attachments
                  </p>
                  `
											: ""
									}
		...
		await Promise.all(emailPromises);
		console.log(
			`Γ£ô Sent ${emailPromises.length} message notifications for group ${groupData.name}`
		);
		return { success: true, emailsSent: emailPromises.length };
	} catch (error) {
...
async function sendGroupCreatedNotification(creatorEmail, groupData) {
	try {
        ...
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 24px;">≡ƒÄë Group Created Successfully!</h1>
          </div>
          
          <div style="background-color: #ffffff; padding: 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 10px 10px;">
            <p style="font-size: 16px; color: #374151; line-height: 1.6;">
              Congratulations! Your study group <strong>${
								groupData.name
							}</strong> is now ready.
            </p>
            
            <div style="background-color: #ecfdf5; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
              <p style="margin: 0 0 10px 0; color: #1f2937;"><strong>≡ƒôÜ Group Name:</strong> ${
								groupData.name
							}</p>
              <p style="margin: 0 0 10px 0; color: #1f2937;"><strong>≡ƒôû Class:</strong> ${
								groupData.className || "N/A"
							}</p>
              <p style="margin: 0 0 15px 0; color: #1f2937;"><strong>≡ƒÅ½ School:</strong> ${
								groupData.school || "N/A"
							}</p>
              <div style="background-color: #ffffff; padding: 15px; border-radius: 6px;">
                ...
                <p style="margin: 0; font-size: 32px; font-weight: bold; color: #10b981; letter-spacing: 2px; font-family: monospace;">${
									groupData.code
								}</p>
              </div>
            </div>
      ...
      `,
		...
		console.log(`Γ£ô Sent group creation confirmation to ${creatorEmail}`);
		return { success: true };
	} catch (error) {
